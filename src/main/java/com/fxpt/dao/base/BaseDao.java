...
public class BaseDao {
    ...
    public <T> List<T> queryApp(String sql, Class<T> beanClass, Object[] params) {
        return jdbcTemplate.query(sql,params,new BeanPropertyRowMapper<T>(beanClass));
    }
    //查询byIDbbbbb
    public <T> T queryAppObjectById(String sql, Class<T> beanClass, Object... ids) {
        try {
    ...
    //分页查询
    public <T extends BaseDTO> Page<T> queryByPage(String sql, Class<T> beanClass, Object[] params, Integer pagesize, Integer count) {
    ...
    //分页查询
    public <T extends BaseDTO> Page<T> xinZenFenYe(String sql, Class<T> beanClass, Object[] params, Integer pagesize, Integer count) {
        Page<T> pageList = new Page<T>();
        int from = pagesize;
        count = (count > 0) ? count : Integer.MAX_VALUE;
//        Pattern p = Pattern.compile("order\\s*by.*");
...
//        }
        String sqlcount="select count(*) total  from ";
        sqlcount=sqlcount+sqlpart;
//        System.out.println("sql语句="+sqlcount);
        int total=this.queryForObject(sqlcount, Integer.class, params);
        if(total<1)
            return new Page<T>();
        List<T> result = new ArrayList<T>();
    ...
    //修改
    public int updateById(String sql, Integer id){
