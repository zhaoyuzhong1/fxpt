...
public class MaterialDao {
    ...
    public Integer addMaterial(Material material){
        String sql =" insert into t_material (name,imgpath,cdate,cuser,flag) values(:name,:imgpath,now(),:cuser,'0')";
    ...
    public Page<Material> getList(String search_name,Integer pagesize, Integer count) {
        String sql = "select tm.*,DATE_FORMAT(tm.cdate,'%Y-%m-%d %H:%i:%s') as ccdate,tu.name as username from t_material tm left join t_user tu on tm.cuser=tu.id where tm.name like ? and tm.flag='0' order by tm.cdate desc ";
